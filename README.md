...
The current portable build is generated locally as:

```text
D:\Codex\output\lan-file-transfer-portable\LanTransfer-Windows.zip
```

For a public GitHub release, upload `LanTransfer-Windows.zip` as a release asset and use:

```text
https://github.com/<owner>/<repo>/releases/latest/download/LanTransfer-Windows.zip
```
